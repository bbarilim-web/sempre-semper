...
import { useAuth, useSchedules, usePinnwand, useSettings } from "./useFirebase";
...
export default function App() {
  ...
  // ── Firebase hooks ──
  const { authUser, profile, loginWithGoogle, logout: fbLogout, saveProfile } = useAuth();
  const { scheds, loaded, saveEvent, deleteEvent, saveAllScheds } = useSchedules(SEED, authUser?.uid);
  const { pinnwand, savePost, deletePost, updatePost } = usePinnwand(SEED_PINNWAND, authUser?.uid);
  const { settings, saveSettings } = useSettings(authUser?.uid);

  // Map Firebase auth user + Firestore profile → app user object
  const user = authUser && profile ? { ...profile, id: authUser.uid } : null;

  // During initial registration profile may not exist yet
  const [registering, setRegistering] = useState(false);
  const [regName, setRegName] = useState("");
  const [regPart, setRegPart] = useState("");

  const saveScheds = async (events) => { await saveAllScheds(events); };
  const savePinnwand = async (posts) => {
    // For bulk updates from PinnwandView we diff and save changed ones
    for (const p of posts) { await savePost(p); }
  };
  const login = async () => {
    try {
      const u = await loginWithGoogle();
      // Profile will load via onSnapshot
    } catch(e) { toast("Anmeldung fehlgeschlagen"); }
  };
  const logout = () => { fbLogout(); setTab("calendar"); };

  const changedCount = scheds.filter(e => e._edited && Date.now() - e.updatedAt < 48 * 3600000).length;

  // ── Loading state — timeout after 4s so we never get stuck ──
  ...
  if (authUser === undefined && !authTimedOut) return (
  ...
  // ── Logged in but no profile yet (first time) → registration ──
  ...
  const navItems = [
    { id:"calendar",  icon:"📅", label:"Spielplan" },
    { id:"vorst",     icon:"🎭", label:"Vorstellungen" },
    { id:"pinnwand",  icon:"📌", label:"Pinnwand", badge: unreadPinn },
    { id:"einstellungen", icon:"⚙", label:"Einstellungen" },
    ...(isAdmin ? [{ id:"admin-panel", icon:"🛠", label:"Admin" }] : []),
  ];
    ...
    <div className="login-wrap">
      <div className="login-card">
        ...
        {step === "main" && <>
          ...
          <div style={{ marginTop:20, padding:"12px 14px", background:"var(--s2)",
            border:"1px solid var(--border)", borderRadius:10,
            fontSize:"0.76rem", color:"var(--faint)", lineHeight:1.6, textAlign:"center" }}>
            Mitglieder des Staatsopernchors melden sich<br/>mit ihrem Google-Konto an.
          </div>
        </>}
